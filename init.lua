...
-- Function to automatically run bun scripts
local function run_bun_script()
  ...
  if not package_json or not package_json.scripts then
    print 'No scripts in package.json found'
    return
  end

  local scripts = {}
  for name, _ in pairs(package_json.scripts) do
  ...
  vim.ui.select(scripts, {
    ...
    if choice then
      vim.cmd('split | term bun run ' .. choice)
    end
  end)
end

vim.keymap.set('n', '<leader>br', run_bun_script, {
  desc = '[B]un [R]un script',
})
